...
| Journal Entry | `day set-property` | `day clear-property` |

Journal entries only support global properties (not category-scoped) and require `-e journal_entry` when creating the property definition. Use `day show` to view all properties for a day. See the [Journal Entries Guide](/docs/guides/journal-entries) for details.
