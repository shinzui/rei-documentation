...
| `path` | File path |
| `url` | URL |
| `note` | Reference to a note entity |
| `git-ref` | Git commit SHA or SHA range |
| `uuid` | UUID (v4, v7, or any) |
| `tag-set` | Free-form tag set (no predeclared values) |
...
### Tag-Set Property (Free-Form Multi-Value)

Tag-set is a multi-value property like label-set, but without a predeclared list of allowed values — any non-empty tag is accepted.
...
### Inheritable Property in the Intention Tree

```bash
rei custom-property create focus-area \
  --type-text --inheritable \
  -e intention
```

See [Inheritance in the Intention Tree](#inheritance-in-the-intention-tree) below for semantics.
...
# Set a tag-set property (comma-separated, free-form)
rei intention set-property <id> topics haskell,event-sourcing
...
**Idempotency:** `set-property` writes no event when the new value equals the current value. Setting a state-machine value to its current state is also a no-op. This is safe to call repeatedly from automation.

## Append and Remove (Multi-Value Properties)

For path-list, label-set, and tag-set properties you can grow or shrink the value one element at a time, without read-modify-write. Each value is a separate shell argument, so commas inside a value are preserved literally (no comma-splitting).

```bash
# Append a single element
rei intention append-property <id> master-plans /abs/path/plan.md

# Append multiple elements at once
rei intention append-property <id> services backend frontend

# Append a value containing commas (treated as one element)
rei intention append-property <id> tags 'comma,inside,one-tag'

# Remove a single element
rei intention remove-property-value <id> tags deprecated

# Remove multiple elements
rei intention remove-property-value <id> master-plans /old/a.md /old/b.md
```

**Idempotency:** `append-property` and `remove-property-value` are idempotent — appending an already-present value or removing an absent value writes no event and exits 0. Re-running the same command against the same target is safe.

**Restrictions:** Only path-list, label-set, and tag-set value types are supported. Other types fail with `MultiValueExpected` — use `set-property` for scalar values.
...
# LabelSet / TagSet filtering (any = has any of, all = has all of)
rei intention list --where "areas any backend,frontend"
rei intention list --where "areas all backend,security"
rei intention list --where "topics any haskell,rust"
```
...
Conditions add value-dependent relevance: a property can only be set when another property has a specific value.

### Adding Conditions

```bash
# reading-status can only be set when content-type = article
rei custom-property add-condition reading-status \
  --when content-type=article
...
|----------|--------|---------|
| Equals | `key=value` | `content-type=article` |
| Not equals | `key!=value` | `status!=archived` |
| Greater than | `key>value` | `effort>3` |
| Greater than or equal | `key>=value` | `priority>=high` |
| Less than | `key<value` | `complexity<5` |
| Less than or equal | `key<=value` | `due-date<=2026-04-01` |
| One of (enum, state machine, label set, tag set) | `key~v1,v2` | `lifecycle~completed,failed` |
| Is set | `key?` | `assigned-to?` |
| Is not set | `key!` | `review-notes!` |

### Semantics

- Multiple conditions are AND-combined
- `--force` on `set-property` bypasses condition validation
- When a conditioning property changes and a dependent property's condition is no longer met, the dependent property is automatically cleared
...
Mark a property inheritable at creation:

```bash
rei custom-property create focus-area --type-text --inheritable
```

Toggle on an existing property:
...
- Walks up the parent intention chain from the target.
- Nearest ancestor wins. A local value always wins over any ancestor.
- Only inheritable properties surface ancestor values; non-inheritable properties show local values only.
- Matching is by `property_id`, not key — values never leak across category-scoped definitions that share the same key.
- Clearing a local value reverts to the inherited one (if any).
- Archived or completed ancestors still contribute values.

State-machine properties cannot be inheritable. They represent per-entity lifecycle position and are rejected at create time and via `set-inheritable`.

### Observing inherited values

```bash
rei intention set-property <root-id> focus-area Product
rei intention show <child-id> --full
# Custom Properties section renders:
#   focus-area: Product (inherited from "Root Intention Title")

rei intention show <child-id> --json
# Inherited rows carry: inherited, inheritedFromId, inheritedFromTitle
```

Filter queries match local values only — `rei intention list --where k=v` does not follow the inheritance path.
...
| `custom-property set-inheritable` | Mark property as inheritable in intention tree |
| `custom-property unset-inheritable` | Stop propagating values to descendants |
...
| Command | Description |
|---------|-------------|
| `custom-property add-condition` | Add a value condition |
| `custom-property remove-condition` | Remove a value condition |
...
Each entity supports the same four-command shape:

| Command | Description |
|---------|-------------|
| `set-property` | Set or replace a value (idempotent) |
| `clear-property` | Remove the local value |
| `append-property` | Add elements to a multi-value property |
| `remove-property-value` | Remove elements from a multi-value property |

Entities with custom-property support:

| Entity | Command prefix |
|--------|----------------|
| Intention | `rei intention …` |
| Habit | `rei habit …` |
| Disruption | `rei disruption …` |
| Note | `rei note …` |
| Day (journal entry) | `rei day …` (today or `--date`) |
| Collection | `rei collection …` |
| Link | `rei link …` |
| Action | `rei action …` |
| Outcome | `rei outcome …` |
| Blocker | `rei blocker …` |
...
| Command | Description |
|---------|-------------|
| `day show` | View all properties for a day |

Journal entries only support global properties (not category-scoped) and require `-e journal_entry` when creating the property definition. See the [Journal Entries Guide](/docs/guides/journal-entries) for details.

## Related

- [State Machine Properties](/docs/guides/state-machines) — Detailed guide for state machines
- [Journal Entries](/docs/guides/journal-entries) — Daily tracking with journal entry properties
