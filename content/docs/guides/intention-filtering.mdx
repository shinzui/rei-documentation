...
### LabelSet / TagSet Membership

Filter by label-set or tag-set property membership using `any` (has any of) or `all` (has all of) operators. The same syntax applies to both property types:
...
rei intention list --where "topics any haskell,rust"  # Same syntax for tag-set
```
