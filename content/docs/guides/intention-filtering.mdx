...
The `rei intention list` command supports multiple filters to narrow down results. Filters combine with **AND** logic — all specified conditions must be satisfied for an intention to appear in the results.
...
Filter to intentions that have a property set (any value):
...
This matches any intention that has the specified property assigned, regardless of its value.
