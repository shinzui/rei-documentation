---
title: State Machine Properties
description: Advanced workflow management with enforced transitions, WIP limits, lifecycle automation, and agent-aware state tracking
icon: Workflow
---

State machine properties are one of rei's most powerful features for managing complex work. They bring workflow discipline (enforced transitions, WIP limits, checklist-gated gates, and lifecycle automation) while giving agents structured context about where every intention stands.

Because agents see state machine values, they can reason about workflow: coaching suggestions that account for what's blocked vs. in-progress, delegation that respects workflow gates, and scheduled checks that react to state changes.

## Overview

A state machine property defines:

| Component | Description |
|-----------|-------------|
| **states** | Named workflow stages (e.g., backlog, in-progress, done) |
| **transitions** | Allowed movements between states |
| **initial state** | Starting state for new assignments |
| **tags** | Semantic labels for filtering (queue, active, done, terminal) |
| **WIP limits** | Advisory limits on how many items can be in a state |
| **state rules** | Required property values to enter a state |
| **state notes** | Phase-specific documentation attached to each state |

## Creating a State Machine

### From a YAML File

```bash
rei custom-property create workflow \
  --type-state-machine --from-file kanban.yaml \
  -e intention
```

### Scoped to a Category

```bash
rei custom-property create workflow \
  --type-state-machine --from-file work-kanban.yaml \
  -e intention -c work
```

## YAML Configuration

Example `kanban.yaml`:

```yaml
initialState: backlog
states:
  - stateId: backlog
    stateLabel: "Backlog"
    stateOrder: 1
    stateTags:
      - systemTag: TagQueue
  - stateId: in-progress
    stateLabel: "In Progress"
    stateOrder: 2
    stateLimit: 3
    stateTags:
      - systemTag: TagActive
  - stateId: done
    stateLabel: "Done"
    stateOrder: 3
    stateTags:
      - systemTag: TagDone
      - systemTag: TagTerminal
transitions:
  - transitionFrom: backlog
    transitionTo: in-progress
    transitionLabel: "Start"
  - transitionFrom: in-progress
    transitionTo: done
    transitionLabel: "Complete"
  - transitionFrom: in-progress
    transitionTo: backlog
    transitionLabel: "Return to Backlog"
```

### State Options

| Option | Required | Description |
|--------|----------|-------------|
| `stateId` | Yes | Unique identifier (lowercase, hyphens allowed) |
| `stateLabel` | Yes | Human-readable display name |
| `stateOrder` | No | Display order (lower = first) |
| `stateDescription` | No | Description of the state |
| `stateLimit` | No | WIP limit (advisory, not enforced) |
| `stateTags` | No | System or user tags for filtering |

## State Tags

System tags provide semantic meaning:

| Tag | Meaning |
|-----|---------|
| `TagQueue` | Items waiting to be worked on (backlog, to-do) |
| `TagActive` | Items currently in progress |
| `TagDone` | Items completed |
| `TagTerminal` | Final states (no further transitions expected) |

Tags enable filtering:

```bash
rei intention list --state-tag active
rei intention list --state-tag done
```

User-defined tags are also supported:

```yaml
stateTags:
  - userTag: blocked
  - userTag: expedited
```

## Transitions

Transitions define allowed state changes. If the transitions list is empty, any state-to-state movement is allowed (free-form workflow).

Transitions are enforced:

```bash
rei intention set-property <id> workflow in-progress   # OK from backlog
rei intention set-property <id> workflow done          # ERROR if no direct path
```

The initial state can always be set on entities without a current value.

## Assigning State Machine Properties

```bash
# Set state on an entity
rei intention set-property <id> workflow in-progress

# Clear property from entity
rei intention clear-property <id> workflow
```

Setting a state to its current value is a no-op — `set-property` is idempotent and writes no event when the new state equals the current one. Safe to call repeatedly from automation.

`append-property` and `remove-property-value` do not apply to state-machine properties (they are scalars). Calling either against a state-machine value fails with `MultiValueExpected`.

When multiple properties share the same key (category-scoped), resolution is automatic based on the entity's category.

## WIP Limits

WIP (Work In Progress) limits are advisory. They show warnings but don't block transitions.

### Setting Limits

```bash
# Set a limit
rei custom-property set-state-limit workflow --state in-progress --limit 3

# Clear a limit
rei custom-property clear-state-limit workflow --state in-progress
```

### Viewing Counts

```bash
rei custom-property show workflow --with-counts
```

Output shows limit status:

```
in-progress - In Progress (limit: 3) [count: 4] OVER LIMIT
backlog - Backlog [count: 5]
done - Done [count: 12]
```

## Evolving State Machines

```bash
# Add a state
rei custom-property add-state workflow blocked --label "Blocked" --tags blocked

# Remove an unused state
rei custom-property remove-state workflow blocked

# Add a transition
rei custom-property add-transition workflow in-progress blocked

# Remove a transition
rei custom-property remove-transition workflow in-progress blocked

# Update display order
rei custom-property update-state-order workflow blocked --order 3
```

## State Rules (Required Properties)

States can require specific property values before an entity may transition into them. This enables checklist-gated workflows where prerequisites must be satisfied before advancing.

### Adding Rules

```bash
# Require deploy-ready=true before entering production state
rei custom-property add-state-rule workflow \
  --state production --require deploy-ready=true

# Remove a rule
rei custom-property remove-state-rule workflow \
  --state production --property deploy-ready
```

When a user attempts to transition into a state with rules, each required property is checked against the entity's current values. If any rule is unsatisfied, the transition is rejected with an error listing the unmet requirements.

### Supported Value Types

| Rule | Type |
|------|------|
| `--require key=true` | Boolean (auto-creates property if needed) |
| `--require key=42` | Integer |
| `--require key=some-text` | Text or enum value |

### Auto-Creation of Boolean Properties

When adding a rule with a boolean value (e.g., `--require deploy-ready=true`), if the referenced property doesn't exist, it is automatically created as a boolean property inheriting the state machine's scope. Non-boolean values still require the property to exist beforehand.

### Example: Checklist-Gated Workflow

1. Add a state rule (auto-creates `deploy-ready` as a bool property):

```bash
rei custom-property add-state-rule workflow \
  --state production --require deploy-ready=true
```

2. Create a checklist note on the intention:

```bash
rei note new -i <id> --category system/checklist
```

3. Link the note's tasks to the boolean property:

```bash
rei intention link-tasks -i <id> \
  --note <note-id> --property deploy-ready
```

When all tasks in the checklist are completed, `deploy-ready` is automatically set to `true`, unblocking the transition to production.

## Task Completion Links

Boolean properties can be automatically set based on task completion in linked notes. When all tasks in a linked note are complete, the property is set to `true`. When any task is reopened, it reverts to `false`.

### Linking Tasks

```bash
# Link a note's tasks to a boolean property
rei intention link-tasks -i <id> \
  --note <note-id> --property <property-key>

# Remove the link
rei intention unlink-tasks -i <id> \
  --note <note-id> --property <property-key>
```

### Behavior

| Condition | Result |
|-----------|--------|
| All tasks complete | Property set to `true` |
| Any task reopened | Property set to `false` (if currently true) |
| No tasks in note | No change (avoids premature true) |

The task completion reactor subscription must be running for automatic property updates.

## State Notes

State notes attach phase-specific documentation to each state of an intention's workflow. Each (intention, property, state) combination can have at most one note, providing a structured place to capture context as an intention moves through workflow stages.

### Opening and Creating State Notes

```bash
# Open or create a state note for the current state
rei custom-property workflow note -i <id>
```

The command determines the intention's current state automatically and opens the note in your editor. If no state note exists yet, one is created.

```bash
# Override the target state
rei custom-property workflow note -i <id> --state in-progress

# Print the note path without opening the editor
rei custom-property workflow note -i <id> --print

# Add tags when creating
rei custom-property workflow note -i <id> --tag planning --tag scope
```

### Listing State Notes

```bash
# List all state notes for an intention
rei custom-property workflow notes -i <id>
```

Output shows one line per state that has a note:

```
State Notes for workflow (intention_01abc...)

  backlog: note_01xyz (created 2026-02-01 14:30)
  in-progress: note_01def (created 2026-01-15 10:00)
```

```bash
# Filter to a specific state
rei custom-property workflow notes -i <id> --state in-progress

# Disambiguate category-scoped properties
rei custom-property workflow note -i <id> --category work
```

State notes are currently supported for intentions only.

## Category-Scoped State Machines

The same state machine key can exist in different categories with different configurations:

```bash
rei custom-property create workflow --from-file work.yaml -c work
rei custom-property create workflow --from-file personal.yaml -c personal
```

Entity assignment automatically uses the correct property based on category:
- Work-categorized intention uses workflow from Work category
- Personal-categorized intention uses workflow from Personal category

Use `--category` to disambiguate in CLI commands:

```bash
rei custom-property show workflow --category work
rei custom-property metrics workflow --category personal
```

## Metrics

View state machine metrics:

```bash
rei custom-property metrics workflow
```

Detailed breakdown by entity:

```bash
rei custom-property metrics workflow --detailed
```

Metrics show:
- Current state distribution with entity counts
- Average time in each state
- Recent transitions (last 7 days)

## Lifecycle Properties (Auto-Complete)

A state machine property can be designated as a "lifecycle property" for intentions. When an intention's lifecycle property reaches a terminal state (tagged with `TagTerminal`), the system schedules automatic completion of the intention after a configurable delay.

### Designating a Lifecycle Property

```bash
# Set lifecycle with 3-day delay
rei custom-property set-lifecycle workflow --days 3

# Use --category for category-scoped properties
rei custom-property set-lifecycle workflow --days 5 --category work
```

This means: when any intention's `workflow` property enters a terminal state, schedule auto-completion in 3 days.

### Removing Lifecycle Designation

```bash
rei custom-property revoke-lifecycle workflow
```

### Requirements

- The property must be a state machine type
- At least one state must have the `TagTerminal` tag
- Days must be >= 1
- Only one lifecycle property is allowed per scope (global or per category)

### Behavior

- When an intention enters a terminal state, a reminder is created for auto-completion after the configured number of days
- If the intention moves back to a non-terminal state, the reminder is cancelled automatically
- If the intention is manually completed, the reminder is also resolved
- The CLI shows a notification when setting a terminal state: "This intention will auto-complete in 3 day(s)"

### Viewing Configuration

```bash
rei custom-property show workflow --with-counts
```

The output includes a Lifecycle section showing the auto-complete delay when configured.

The `terminal-state-completion` subscription must be running for automatic completion scheduling.

## Auto-Initialization

When an entity is assigned to a category, state machine properties scoped to that category are automatically initialized with their initial state.

For existing entities, manually initialize:

```bash
rei custom-property init-entities workflow
```

## Command Summary

### Property Definition

| Command | Description |
|---------|-------------|
| `custom-property create` | Create state machine from YAML |
| `custom-property show` | Show states and transitions |
| `custom-property show --with-counts` | Include entity counts per state |

### State Evolution

| Command | Description |
|---------|-------------|
| `custom-property add-state` | Add a new state |
| `custom-property remove-state` | Remove an unused state |
| `custom-property add-transition` | Add a transition rule |
| `custom-property remove-transition` | Remove a transition rule |
| `custom-property update-state-order` | Set display order for a state |

### State Rules

| Command | Description |
|---------|-------------|
| `custom-property add-state-rule` | Require a property value to enter a state |
| `custom-property remove-state-rule` | Remove a required property rule |

### WIP Limits

| Command | Description |
|---------|-------------|
| `custom-property set-state-limit` | Set WIP limit on a state |
| `custom-property clear-state-limit` | Remove WIP limit |

### Task Completion Links

| Command | Description |
|---------|-------------|
| `intention link-tasks` | Auto-set property when tasks complete |
| `intention unlink-tasks` | Remove task completion link |

### State Notes

| Command | Description |
|---------|-------------|
| `custom-property <key> note` | Open or create a state note |
| `custom-property <key> notes` | List state notes for an entity |

### Lifecycle

| Command | Description |
|---------|-------------|
| `custom-property set-lifecycle` | Designate as lifecycle property |
| `custom-property revoke-lifecycle` | Remove lifecycle designation |

### Metrics

| Command | Description |
|---------|-------------|
| `custom-property metrics` | View workflow metrics |

### Assignment

| Command | Description |
|---------|-------------|
| `intention set-property` | Set state on intention |
| `intention clear-property` | Clear state from intention |
| `habit set-property` | Set state on habit |
| `disruption set-property` | Set state on disruption |

## Related

- [Custom Properties](/docs/guides/custom-properties): General custom property information
- [`rei custom-property`](/docs/commands/custom-property): Full command reference
