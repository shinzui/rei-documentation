...
- `--no-guidance` — Skip intention-level note guidance
...
If the note's anchor intention has note guidance set (via `intention set-note-guidance`), that guidance is automatically included as context. Use `--no-guidance` to skip this.
