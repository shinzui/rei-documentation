...
**Scope Filters:**
- `-i, --intention <id>` — Filter by intention ID (optional if fzf is available)
...
Set a custom property on an action.

```bash
rei action set-property [-a ACTION_ID] PROPERTY_KEY VALUE [--force]
```
...
- `--at <time>` — When the change actually happened (e.g. `"2 hours ago"`, `-1h`, `2024-01-15T14:30:00Z`). Defaults to now. Distinct from `record --at`, which sets the action's completion time.
...
```bash
# Set property with explicit action ID
rei action set-property -a action_01h455vb4pex mood focused

# Let fzf pick the action
rei action set-property mood focused

# Set a numeric property
rei action set-property -a action_01h455vb4pex effort 5

# Skip intermediate states on a state machine property
rei action set-property -a action_01h455vb4pex debug-state resolved --force

# Backdate a state transition
rei action set-property -a action_01h455vb4pex debug-state resolved --at "1 hour ago"
```

**Notes:**
- The property must be defined first using `rei custom-property create`
- The property must be configured to apply to actions (`--entity-type action`)
...
- `--at` records the **effective time** of the property change. It is independent of the action's `occurredAt` (set via `record --at`). `rei custom-property metrics` uses the property's effective time for cycle-time and lead-time math.
- See [`rei custom-property`](/docs/commands/custom-property) for how to define properties and how category-scoped state machine properties are auto-initialized when an action is categorized.

### clear-property

Remove a custom property from an action.

```bash
rei action clear-property [-a ACTION_ID] PROPERTY_KEY
```
...
```bash
# Clear property with explicit action ID
rei action clear-property -a action_01h455vb4pex mood

# Let fzf pick the action
rei action clear-property mood
```
...
```bash
rei action append-property [-a ACTION_ID] PROPERTY_KEY VALUE [VALUE...]
```

Each `VALUE` is a separate shell argument; commas are preserved literally. See [`rei intention append-property`](/docs/commands/intention) for the full description and examples.
...
```bash
rei action remove-property-value [-a ACTION_ID] PROPERTY_KEY VALUE [VALUE...]
```
