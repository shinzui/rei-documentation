...
> **FZF Support:** If `-i` is not provided and [fzf](https://github.com/junegunn/fzf) is installed, an interactive picker will let you select from active intentions. After selecting an intention, another picker will let you optionally select a habit (with a "Skip" option at the top). Use `--no-habit` to skip the habit selection prompt entirely.
