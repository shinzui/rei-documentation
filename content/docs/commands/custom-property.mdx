...
Define and manage custom properties that can be attached to entities (intentions, habits, disruptions, notes, journal entries, collections, links). Custom properties allow you to extend entities with your own metadata fields.
...
- `-e, --entity-type <type>` — Filter by entity type (intention, habit, disruption, note, collection, link)
- `-c, --category <slug>` — Category scope for disambiguation when multiple properties have the same key
- `--json` — Output as JSON
...
**Important distinction:** what is inherited is the *category context* used to decide which property definition applies — **not** property values. Setting `priority=high` on a parent intention does NOT propagate that value to its children; each intention still holds its own property values.
