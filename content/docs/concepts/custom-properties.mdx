...
description: Shared structured context between humans and agents, typed metadata that powers smarter collaboration
icon: Settings
---

Custom properties are how you and your agents share structured context. They're typed metadata fields you define on entities, and because agents see them, they become a communication channel between human intent and agent understanding.

When you set `project_path` on an intention, the agent knows where to look. When you define a `workflow` state machine, the agent understands where each intention stands. When you track `swimming_distance` on a habit, the agent can reason about trends. **Properties turn implicit context into explicit, queryable, agent-readable facts.**

## Why Custom Properties?

### Shared Human-Agent Context

Properties are the bridge between your knowledge and agent capability. Agents see every property value when coaching, assisting, or executing delegated work.

**Example:** An intention has a `project_path` property pointing to where the related project lives on the filesystem. When you delegate research, the agent knows exactly where to look. When you ask for coaching, the agent can reference actual code and files.

**Example:** A `repo_url` property on an intention means the agent can look at PRs, issues, and recent commits when reviewing your progress.

### Advanced Workflow Management

State machine properties bring full workflow discipline: defined states, enforced transitions, WIP limits, lifecycle automation, and checklist-gated gates. See the [State Machine Properties](/docs/guides/state-machines) guide for the complete picture.

**Example:** A kanban-style `workflow` with states from backlog through review to done, where transitions are enforced, WIP limits are advisory, and reaching a terminal state auto-completes the intention after a configurable delay.

### Structured Tracking

Track quantifiable data on entities that enables queries, analysis, and trend-aware coaching.

**Example:** A habit tracks `swimming_distance` with units (meters, km), allowing queries like "total distance this month" and enabling the agent to notice patterns like "you've been increasing distance steadily, ready for the next goal?"
